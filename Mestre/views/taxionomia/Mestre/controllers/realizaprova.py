...
def realizar_prova():
 ...
 if 'auth' in globals():
  if auth.is_logged_in():
    ...
    #Para verificar como o python monta a query
    print db(db.prova.turma==idTurma)._select(db.prova.id,db.prova.referencia,db.prova.plano_de_prova)
    #Para verificar o que a query esta trazendo. Ajuda no Debug
    print row_prova 
    if row_prova:
            for pl in row_prova:
                idProva=pl.id
    ...
    #Para verificar como o python monta a query
    print db(db.plano_de_prova.id==idPlanoProva)._select(db.plano_de_prova.id,db.plano_de_prova.referencia)
    #Para verificar o que a query esta trazendo. Ajuda no Debug
    print row_planoprova
    if row_planoprova:
