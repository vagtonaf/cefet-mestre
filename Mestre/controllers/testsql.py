...
def testsql():
    results = db.executesql("""SELECT c.referencia as prova, g.referencia as plano_de_prova, e.first_name,  e.last_name, c.data_aplicacao, b.data as data_conclusao, b.gerada, f.enunciado, h.resposta, h.correta, i.valor
    ...
    totaliza = db.executesql("""SELECT c.referencia as prova, g.referencia as plano_de_prova, e.first_name,  e.last_name, sum(i.valor) as total
                                           FROM item_prova_gerada as a  
                                           left join prova_gerada as b on a.prova_gerada==b.id 
                                           left join prova as c on b.prova==c.id 
                                           left join aluno as d on b.aluno==d.id 
                                           ...
                                           left join alternativa as h on a.alternativa_escolhida==h.id
                                           left join item_plano_de_prova as i on (f.taxionomia==i.taxionomia and f.dificuldade==i.dificuldade and f.topico==i.topico and c.plano_de_prova==i.plano_de_prova) 
                                           left join alternativa as j on j.questao==f.id  where j.correta==h.correta group by c.referencia, g.referencia, e.first_name,  e.last_name
                                           """)
